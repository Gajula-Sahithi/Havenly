#!/bin/bash

echo "🚀 Deploying DormFlow to Vercel..."

# Set environment variables
export FIREBASE_PROJECT_ID="dormlink-ec53d"
export FIREBASE_CLIENT_EMAIL="firebase-adminsdk-fbsvc@dormlink-ec53d.iam.gserviceaccount.com"
export FIREBASE_PRIVATE_KEY="-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
export FIREBASE_DATABASE_URL="https://dormlink-ec53d.firebaseio.com"
export JWT_SECRET="dormlink_super_secret_jwt_key_2024_secure_production"
export GEMINI_API_KEY="AIzaSyChBeZwGc64e3xtJuxkGk36jU-8e-njUu0"

# Deploy to Vercel
echo "📦 Building and deploying..."
vercel --prod

echo "✅ Deployment complete!"
