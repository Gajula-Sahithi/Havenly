# 🔧 Fixed Environment Variables for Vercel

## ✅ Corrected Format (Copy-Paste This):

```
FIREBASE_PROJECT_ID=dormlink-ec53d
FIREBASE_CLIENT_EMAIL=firebase-adminsdk-fbsvc@dormlink-ec53d.iam.gserviceaccount.com
FIREBASE_PRIVATE_KEY=-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
FIREBASE_DATABASE_URL=https://dormlink-ec53d.firebaseio.com
JWT_SECRET=dormlink_super_secret_jwt_key_2024_secure_production
GEMINI_API_KEY=AIzaSyChBeZwGc64e3xtJuxkGk36jU-8e-njUu0
```

## 🔧 Key Changes:

1. **Removed quotes** around the private key
2. **Used actual line breaks** instead of `\n`
3. **Fixed format** for Vercel environment variables

## 🚀 Deploy Again:

```bash
vercel --prod
```

When prompted, copy-paste the entire block above exactly as shown.

## ✅ Error Fixed:

- ❌ **Before**: Private key with quotes and \n
- ✅ **After**: Private key with actual line breaks, no quotes

**This will fix the Firebase initialization error in Vercel!**
