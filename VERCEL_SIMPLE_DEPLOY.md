# 🚀 Simple Vercel Deployment - Fixed

## 🔧 Issue: Firebase Private Key Format

The problem is Vercel doesn't handle multi-line environment variables well. Let's use a different approach.

## ✅ Solution 1: Use Single-Line Private Key

Replace the multi-line private key with this single-line version:

```
FIREBASE_PROJECT_ID=dormlink-ec53d
FIREBASE_CLIENT_EMAIL=firebase-adminsdk-fbsvc@dormlink-ec53d.iam.gserviceaccount.com
FIREBASE_PRIVATE_KEY=-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n
FIREBASE_DATABASE_URL=https://dormlink-ec53d.firebaseio.com
JWT_SECRET=dormlink_super_secret_jwt_key_2024_secure_production
GEMINI_API_KEY=AIzaSyChBeZwGc64e3xtJuxkGk36jU-8e-njUu0
```

## ✅ Solution 2: Use Vercel Dashboard (Recommended)

1. Go to vercel.com
2. Create New Project
3. Connect your GitHub repository
4. In Environment Variables section, add each variable separately:
   - FIREBASE_PROJECT_ID: dormlink-ec53d
   - FIREBASE_CLIENT_EMAIL: firebase-adminsdk-fbsvc@dormlink-ec53d.iam.gserviceaccount.com
   - FIREBASE_PRIVATE_KEY: [Paste the entire private key with actual line breaks]
   - FIREBASE_DATABASE_URL: https://dormlink-ec53d.firebaseio.com
   - JWT_SECRET: dormlink_super_secret_jwt_key_2024_secure_production
   - GEMINI_API_KEY: AIzaSyChBeZwGc64e3xtJuxkGk36jU-8e-njUu0

## 🚀 Deploy:

```bash
vercel --prod
```

## ✅ This fixes the Firebase initialization error!

The key is using the dashboard for environment variables or the single-line format.
